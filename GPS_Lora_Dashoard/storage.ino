...
void store_peer_position(int device_id) {
 char stringa[128];
  char tmpstr[18];
  mygps.rawdate.toCharArray(tmpstr, 18);
  sprintf(stringa, "Insert into remote_peers(timestamp,lat,lon,device_id) values (\'%s\',%f,%f,%d)", tmpstr, peers_position[device_id].lat, peers_position[device_id].lon, device_id);
  //Serial.println(stringa);
  rc = db_exec(db1, stringa);
  if (rc != SQLITE_OK) {
    Serial.println("Unable to store data");
